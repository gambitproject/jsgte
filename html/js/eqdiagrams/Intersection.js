GTE = (function (parentModule) {
    ...
    Intersection.prototype.draw = function (){
       ...
       GTE.svg.removeChild(this.lines[0]);
       GTE.svg.removeChild(this.lines[1]);
       }
    ...
    */
     ...
     * Change position
     */
    Intersection.prototype.move = function (new_x) {
        this.x = new_x;
       this.point.setAttributeNS(null, "cx", new_x);
       this.label.setAttributeNS(null, "cx", new_x);
       if (this.player==0)
          this.label.value=(Number(new_x)-Number(GTE.diag.margin))/(Number(GTE.width)-2*Nmuber(GTE.margin));
       else
          this.label.value=(Number(new_x)-4*Number(GTE.diag.margin)-Number(GTE.width))/(Number(GTE.width)-2*Nmuber(GTE.margin));
