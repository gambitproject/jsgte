GTE = (function(parentModule) {
       ...
       event.preventDefault();
       this.prev_pos=GTE.getMousePosition(event);
       var strat=event.currentTarget.getAttribute("asso_strat");
       var player=event.currentTarget.getAttribute("asso_player");
       ...
       var mousePosition = GTE.getMousePosition(event)
       var svgPosition = GTE.svg.getBoundingClientRect();
       diff=mousePosition.y-this.prev_pos.y;
       ...
       this.moving_line.addEventListener("mousedown", GTE.diag.doMouseDownLine);
       this.moving_line=null;
       };
       ...
       document.removeEventListener("mouseup", GTE.diag.doMouseupEndpoint);
       this.moving_line.addEventListener("mousedown", GTE.diag.doMouseDownEndpoint);
       this.moving_line=null;
       };
       ...
       for ( var i=0;i<2;i++){
          for (var j=0;j<4;j++){
             this.payoffs[i][j]=(Math.round(GTE.tree.matrix.matrix[j].strategy.payoffs[i].value*precision)/precision);
       ...
       var middle_y =(this.payoffs[i][this.couple_strat(i,0)[1]]-Number(this.payoffs[i][this.couple_strat(i,0)[0]]))*middle_x/this.payoffs[i][this.couple_strat(i,0)[0]];
